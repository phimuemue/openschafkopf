pub fn via_out_param_init<T: std::borrow::BorrowMut<TBorrow>, TBorrow: ?Sized, R, F: FnOnce(&mut TBorrow)->R>(mut t: T, f: F) -> (T, R) {
    let r = f(t.borrow_mut());
    (t, r)
}

pub fn via_out_param_init_result<T: std::borrow::BorrowMut<TBorrow>, TBorrow: ?Sized, R, E, F: FnOnce(&mut TBorrow)->Result<R, E>>(t: T, f: F) -> Result<(T, R), E> {
