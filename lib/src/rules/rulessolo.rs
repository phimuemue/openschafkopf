...
impl TPayoutDeciderSoloLike for SPayoutDeciderPointBased<VGameAnnouncementPrioritySoloLike> {
    ...
    fn points_as_payout(&self, rules: &SRulesSoloLike<Self>) -> Option<(SRules, FnConvertPointsAsPayout)> {
        ...
        Some((
            ...
            Box::new(move |_rulestatecache: &SRuleStateCacheFixed, ekurzlang, epi_hand, n_payout: isize| {
...
impl SPayoutDeciderPointsAsPayout<VGameAnnouncementPrioritySoloLike> {
    fn payout_to_points(epi_active: EPlayerIndex, epi_hand: EPlayerIndex, pointstowin: &impl TPointsToWin, n_payout: isize, ekurzlang: EKurzLang) -> isize {
...
impl TPayoutDeciderSoloLike for SPayoutDeciderTout {
    ...
    fn payout(&self, rules: &SRulesSoloLike<Self>, rulestatecache: &SRuleStateCache, stichseq: SStichSequenceGameFinished, expensifiers: &SExpensifiers) -> EnumMap<EPlayerIndex, isize> {
        ...
        internal_payout(
            /*n_payout_primary_unmultiplied*/((self.payoutparams.n_payout_base + self.payoutparams.laufendeparams.payout_laufende(&rules.trumpfdecider, &rulestatecache.fixed, stichseq.get().kurzlang(), playerparties13)) * 2)
