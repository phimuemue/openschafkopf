...
pub fn with_common_args<FnWithArgs, ItRules>(
    clapmatches: &clap::ArgMatches,
    fn_itrules: impl FnOnce(Option<Result<Box<dyn TRules>, Error>>) -> Result<ItRules, Error>,
    ...
    where
        ...
        ItRules: Iterator<Item=Box<dyn TRules>>,
{
    let b_verbose = clapmatches.is_present("verbose");
    for rules in fn_itrules(super::get_rules(clapmatches))? {
